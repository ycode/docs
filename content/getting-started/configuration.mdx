...
| `SUPABASE_URL` | No | Supabase API gateway URL. Required for self-hosted Supabase (e.g. `https://supabase.my-company.com`). Leave empty for Supabase Cloud. |
...
Ycode manages its own authentication flow and does not use Supabase's email confirmation. You need to disable it before creating your admin account. The setup wizard verifies this setting automatically and prompts you if it is still enabled.
...
1. Open the `.env` file next to your `docker-compose.yml`.
2. Set `ENABLE_EMAIL_AUTOCONFIRM=true`.
...
2. Complete the setup wizard. It connects to Supabase, runs migrations, verifies the email confirmation setting, creates your admin account, and lets you choose a starter template.
