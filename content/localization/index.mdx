...
Published HTML sets `<html lang>` from the URL (`/` uses the default locale, `/fr/...` uses `fr`) and `<html dir="rtl">` or `dir="ltr"` from the language. Ycode also emits `<link rel="alternate" hreflang="...">` tags in the document `<head>` from the first HTML byte, covering each published locale plus an `x-default` for the default language. See [hreflang for Multilingual Sites](/docs/seo#hreflang-for-multilingual-sites) for details.
