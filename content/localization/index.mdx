...
Ycode also emits `<link rel="alternate" hreflang="...">` tags in every page's `<head>` covering each published locale plus an `x-default` for the default language, so search engines cluster the variants of each page correctly and serve the right one in each region. See [hreflang for Multilingual Sites](/docs/seo#hreflang-for-multilingual-sites) for details.
