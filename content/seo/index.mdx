...
Both static pages and dynamic CMS pages get the right URLs — dynamic pages use the translated item slug per locale where one exists, and fall back to the default slug otherwise. Preview, error, and `noindex` pages are excluded. Hreflang, canonical, and `og:url` are omitted when no site base URL is set (`Global Canonical URL` or a primary domain). The same alternates are reused by `sitemap.xml`, so the two surfaces can't drift.
