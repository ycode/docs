...
| **Blur** | Animate a Gaussian blur in pixels (defaults 0 → 5 px) |
| **Brightness** | Animate brightness — `1` is the original image, `0.5` is half as bright, `2` is twice as bright (defaults 1 → 1.15) |
| **Grayscale** | Animate desaturation from 0% (full color) to 100% (fully grayscale) |
| **Display** | Toggle between visible and hidden |

A **Display** tween that goes from hidden to visible also works when the target is hidden via Settings `Visibility` or a Tailwind `hidden` class, including breakpoint-scoped classes such as `max-md:hidden`. Ycode strips and restores the class around the reveal, so a mobile menu can open on small screens without hiding the desktop nav. The on-load / on-trigger choice for Display is unchanged, so existing interactions keep their load-time behavior.

Blur, brightness, and grayscale are filter properties — adding more than one on the same tween composes them (e.g. blur 5px combined with brightness 1.15 plays as a single `filter` declaration), so you can layer effects without one overwriting the next.
...
For **Page load** and **Scroll into view** triggers, the initial state is always applied on page load — Ycode emits the starting CSS in the server-rendered HTML so the element paints in its from-state with no flash. The per-property toggle is shown as locked for these triggers because there is nothing to switch.
...
For click, hover, and page load triggers, choose how the animation behaves:

| Effect | Description | Available on |
|--------|-------------|--------------|
| **Reset and run once** | Plays once, resets to start | Click, Hover, Page load |
| **Toggle and run once** | Plays forward, then reverses on next trigger | Click, Hover |
| **Loop — Reset and restart** | Repeats indefinitely | Click, Hover, Page load |
| **Loop — Toggle and restart** | Plays forward and backward indefinitely (yoyo) | Click, Hover, Page load |

Use the **Loop** effects with **Page load** to run an animation continuously from the moment the page mounts — useful for ambient motion on hero elements, animated backgrounds, and breathing CTAs.
