...
- Canonical, `og:url`, and hreflang when a site base URL is set (`Global Canonical URL` or a primary domain); omitted when it is not
...
- Code Embed (htmlEmbed) layers — your HTML, CSS, and JS render inline at their natural height; pasted `<script>` tags run as authored
- Interactions for the click→display and page-load→display patterns (mobile menu toggles work out of the box)
- Error pages (`401`, `404`, `500`) emitted at the root of the output directory
- HTML comments `Made in Ycode · ycode.com` and `Published … UTC` after the doctype (same stamps as the live site)
- `<meta name="generator" content="Ycode" />`
...
Custom code added in Ycode is emitted into the static export the same way it is on a live SSR site:

- **Global custom code** (Settings → General → Custom Code) is injected into every page's `<head>` and pre-`</body>`.
- **Per-page custom code** (Page Settings → Custom Code) — `Header` lands in the real document `<head>`, `Body` before `</body>`.
- **Dynamic-page placeholders** like `{{Title}}` resolve per item, so each generated CMS item page carries its own resolved snippet.

This covers analytics (GA / GTM / Plausible / PostHog), Google Search Console verification, custom font loaders, structured-data JSON-LD, and any third-party widget tag — all included in the exported HTML, in the same order as on the live site (global first, then per-page). Pages with no custom code ship byte-identical output to previous exports.
...
- Pages whose visibility rules use `Today`, `This week`, `This month`, or `This year` carry an inline boot script that re-evaluates those rules on page load.
- Field values are baked in at export time — only the date reference is resolved in the browser — so layers gated on relative dates flip correctly when the day boundary passes, without re-exporting.
