...
| **option** | Dropdown with predefined choices; renaming or removing an option updates all existing items automatically |
...
| **email** | Email address |
| **phone** | Phone number |
| **link** | Link to an external URL, an internal page, or a downloadable file from the asset manager. Buttons and wrapped elements bound to the field resolve to whichever type each item uses |
...
| Type | Description |
|------|-------------|
| **reference** | Link to a single item in another collection (searchable dropdown) |
| **multi_reference** | Link to multiple items in another collection (searchable dropdown) |
...
| Type | Description |
|------|-------------|
| **count** | Counts how many items in another collection reference the current item via a chosen `reference` or `multi_reference` field. The value updates live as related items are added, removed, or staged for publish. Read-only — counts are not editable on items, and the field is bindable to text layers like a regular number |

To configure a count field, pick the source collection and the reference field that points back to the current collection. Only valid combinations appear in the dropdown.
