...
On dynamic collection pages, you can add links that navigate to the next or previous item. When configuring a link to the same dynamic page, the item selector shows **Next item** and **Previous item** options. Ycode resolves these at render time based on the item order.

You can configure the sort order for next/previous navigation in the page settings under the CMS section (sort by field and direction).

### Previewing Items on the Canvas

While editing a dynamic page, the item picker next to the page selector controls which collection item the canvas renders against. The first item is selected automatically. Type into the search field to find items beyond the preloaded set — Ycode searches the whole collection on the server, so large collections are fully reachable. Switching items updates the canvas, slug, and any bound fields in place.

The same search-and-pick experience powers every CMS item picker in the builder — layer **Link** settings, the rich-text editor's link popover, and a collection's **Link** field — so picking a specific item never gets stuck on the first 25 results.
