...
On Ycode Cloud, restores up to **50 MB** are supported. The browser uploads the backup directly to storage and the import runs by reference, so large projects with many embedded assets restore without hitting request-size limits. Self-hosted Ycode has no fixed cap beyond your own infrastructure.
</Callout>
